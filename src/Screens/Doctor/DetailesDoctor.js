...
function DetailesDoctor({ navigation, route }) {
    ...
    const { name, picture, doctor_type, educations, chembers, description,present_workplace,contact} = route.params.item
                ...
                <View style={{ width: "98%", margin: "auto" }}>
                    ...
                    }
                    {
                        ...
                        <View style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexDirection:"row"}}>
                            <Text>{contact}</Text>
                            <IconButton icon={"phone"} iconColor='white' style={{ borderRadius: 10 }} containerColor='green' onPress={() => Alert.alert("call", `আপনি কি ${name} .... কল করতে চান`, [{ text: "হ্যাঁ!", onPress: () => { Linking.openURL(`tel:${contact}`) } }, { text: "না!", onPress: () => { } }])} />
                        </View>
                    }
                    <View style={{ width: "100%" }}>
                        {educations.length != 0 && <Text style={{ width: "100%", fontSize: 22, color: "#DE1976", marginBottom: 5, borderBottomWidth: 3, borderBottomColor: "#DE1976" }}>শিক্ষা</Text>}
                        {educations.length != 0 &&
                            educations.map((item, index) => (
                                <View key={index}>

                                    <Text style={{fontSize:17,display:"flex",flexDirection:"row",alignItems:"center",justifyContent:"flex-start"}}>
                                        <Icon source={'disc'} /> {item.exam} : {item.institute} - {item.year}
                                    </Text>
                                </View>
                            ))}
                        
                        {chembers.length != 0 && <Text style={{ width: "100%", fontSize: 22, color: "#DE1976", marginBottom: 5, borderBottomWidth: 3, borderBottomColor: "#DE1976" }}>চেম্বার</Text> }
                        {chembers.length != 0 &&
                            chembers.map((item, index) => (
                                <View key={index} style={{paddingLeft:5 ,width: "98%", borderRadius: 5, borderWidth: 0.2, paddingHorizontal: 5, paddingVertical: 2 }} >
                                    <Text style={{ fontSize: 17 }}>চেম্বারের নামঃ {item.chemberName}</Text>
                                    <Text style={{ fontSize: 17 }}>দিনঃ
                                        {item.chemberDay && item.chemberDay.length != 7 ? item.chemberDay.map((item)=>{
                        ...
                        {description && <Text style={{ width: "100%", fontSize: 22, color: "#DE1976", marginBottom: 5, borderBottomWidth: 3, borderBottomColor: "#DE1976" }}>বিবরনী</Text> }
                        {                            
                         description &&   <Pressable style={{marginBottom:20}} onPress={() => setShows((prev) => ({ ...prev, descriptionShow: !prev.descriptionShow }))}>
                                {description && <Text>{iconData.length>300 && shows.descriptionShow ?iconData.slice(0,300):iconData}</Text>}
                            </Pressable>
                        }
                    </View>
                </View>
